...
__device__ double distance(double *d_inputs, double *shared_centroids, int idx1, int idx2, int dims)
{
	double dist = 0;

    for(int i = 0; i<dims; i++){
        dist += (shared_centroids[idx2*dims + i] - d_inputs[idx1*dims + i]) * (shared_centroids[idx2*dims + i] - d_inputs[idx1*dims + i]) ;
    }
...
__global__ void clusterAssignmentShmem(int *d_outputs, double *d_inputs, double *d_centroids, int k, 
                                       int n_vals, int dims, double *holdCentroid, double *numberCluster){
	 ...
	 if (threadIdx.x < k*dims){
		shared_centroids[threadIdx.x] = d_centroids[threadIdx.x];
     }
     ...
     double dist = 0;
        ...
        for (int i = 0; i < k; i++){
           dist = distance(d_inputs, shared_centroids, tid, i, dims);
           if (dist < min_dist){
               min_dist = dist;
               centroid_label = i;     
           } 
...
__global__ void avgCentroidsShmem(double *d_centroids, int k, int dims, double *holdCentroid, double *numberCluster){
   ...
   int *d_outputs;
    ...
    while(!done){
        ...
        cudaMemcpy(prev_centroids, d_centroids, k * dims * sizeof(double), cudaMemcpyDeviceToHost);
         ...
         clusterAssignmentShmem<<<grid_size, T, (k*dims*sizeof(double)) >>>(d_outputs, d_inputs, d_centroids, k, n_vals, dims, hold_centroids, numclusters);
         cudaDeviceSynchronize();
          
         avgCentroidsShmem<<<1, (k*dims)>>>(d_centroids, k, dims, hold_centroids, numclusters);
         cudaDeviceSynchronize();
